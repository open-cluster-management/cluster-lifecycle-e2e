...
if [[ $TEST_GROUP == "import" ]]; then
    ...
    ginkgo -v -focus="create" --nodes=3 --reportFile=$REPORT_FILE_CREATE  -trace -debug create_cluster/create_cluster.test -- -v=3 -owner="ginkgo-$TRAVIS_BUILD_ID" -cloud-providers=aws,azure,gcp
elif [[ $TEST_GROUP == "destroy" ]]; then
    ginkgo -v -focus="detach|destroy" -p --reportFile=$REPORT_FILE_DESTROY -trace -debug detach_destroy/detach_destroy.test -- -v=3 -owner="ginkgo-$TRAVIS_BUILD_ID" -cloud-providers=aws,azure,gcp
