...
if [[ $TEST_GROUP == "import" ]]; then
    ...
    ginkgo -v -focus="create" -skip="" -p -trace --debug e2e-test.test -- -v=3 -owner="ginkgo-$TRAVIS_BUILD_ID" -report-file=$REPORT_FILE_CREATE
elif [[ $TEST_GROUP == "destroy" ]]; then
    ginkgo -v -focus="detach|destroy" -skip="" -p -trace -debug e2e-test.test -- -v=3 -owner="ginkgo-$TRAVIS_BUILD_ID" -report-file=$REPORT_FILE_DESTROY
