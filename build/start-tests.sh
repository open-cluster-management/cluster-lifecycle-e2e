...
if [[ $TEST_GROUP == "import" ]]; then
    ...
    ginkgo -v -focus="create" --nodes=3 --stream -trace -debug create_cluster/create_cluster.test -- -v=3 -owner="ginkgo-$TRAVIS_BUILD_ID" -report-file=$REPORT_FILE_CREATE -cloud-providers=aws,azure,gcp
elif [[ $TEST_GROUP == "destroy" ]]; then
    ginkgo -v -focus="detach|destroy" -p --stream -trace -debug detach_destroy/detach_destroy.test -- -v=3 -owner="ginkgo-$TRAVIS_BUILD_ID" -report-file=$REPORT_FILE_DESTROY -cloud-providers=aws,azure,gcp
