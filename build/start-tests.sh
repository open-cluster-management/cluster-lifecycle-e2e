...
if [[ $TEST_GROUP == "import" ]]; then
    ...
    ginkgo -v -focus="detach|destroy" -p -trace -debug e2e-test.test -- -v=3 -owner="ginkgo-$TRAVIS_BUILD_ID" -report-file=$REPORT_FILE_DESTROY -cloud-providers=aws,azure,gcp
    ...
    ginkgo -v -focus="create" --reportFile=$REPORT_FILE_BM -trace -debug e2e-test.test -- -v=3 -cloud-providers=baremetal
elif [[ $TEST_GROUP == "destroy-baremetal" ]]; then
    ginkgo -v -focus="destroy" --reportFile=$REPORT_FILE_BM -trace -debug e2e-test.test -- -v=3 -cloud-providers=baremetal
fi